...
Python ≥ 3.9；除标准库 + PyYAML 外没有运行时依赖）。下面每条路径都需要你拥有该仓库的访问权限。

```bash
# 1) 从最新 release 下载 wheel（或 .tar.gz），然后安装这个本地文件：
...
> 由于仓库是私有的，未经认证的 `pip install <release-下载-URL>` 形式无法解析 —— 请先从 release
> 页面（你已登录的地方）下载该资产，或从一个 clone 安装。
...
其“隐藏”只是**约定，而非强制**（见
[真实 agent dogfood](#真实-agent-dogfood--保留集-held-out-反馈屏障)）。一个读写双向限制的选项
（在 OS 级文件系统沙箱中运行 agent）是已规划、尚未实现的能力。作为一道护栏：自 v0.3.3 起，放在
...
- **文件系统读取限制 / OS 级 agent 沙箱** —— 目前爆炸半径闸门只约束写入（见[安全模型](#安全模型)）。
